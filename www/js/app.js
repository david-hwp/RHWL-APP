...
function writeBySelf(){
    if(options!="write"){
        ...
        $("#scanner").attr({"placeholder":"请输入订单号码..."});
        $("#scannerLogo").removeAttr("class");
        $("#scannerLogo").addClass("glyphicon glyphicon-barcode");
        options="write";
    }else{
        $("#scanner").attr({"onclick":"wuliu()"});
        $("#scanner").attr({"placeholder":"扫码或点击右侧手动填写"});
        $("#scannerLogo").removeAttr("class");
    ...
    if (evt.type == "intel.xdk.device.barcode.scan") {
        if (evt.success === true) {
            ...
            if(options=="addDemo"){
                ...
                $("#body-one").show();
                $("#confrimOut").show();
//                $("#tbody_y").append("<tr><td>"+(i+1)+"</td><td id='t"+i+"'onclick='changeDemoCode(this)'>"+url+"</td><td><div class='btn-group'><input type='button' class='btn btn-info btn-xs' value='关联'/><input type='button' onclick='delFromDemo(this)' class='btn btn-danger btn-xs' value='删除'></div></td></tr>");
//                $(".demoCount").text(i+1);
//                $(".allCount").text(i+1);
                //i++;
                showDemo();
           ...
           //模拟数据
            var goodsItems={ "neworder01":[{"logIdCompany":"9331708964","Number":"10"}]};
                ...
                cancel=true;
               //$("#recieve").css({"marginTop":"-50px"});
                //$("#logisticsSub").css({"marginTop":"-50px"});
            if(options=="addDemo"||options=="wuliu"){
                 //$("#send").css({"marginTop":"-50px"});
...
function fillDemoCode(obj){
    ...
    var text=obj.value;
    $("#t"+id).html(text);
    $("#t"+id).attr({"onclick":"changeDemoCode(this)"});
    changeFromDemo(preCode,text);
}
...
//从样品代号序列中修改一个样品
function changeFromDemo(preCode,afterCode){
    for(var i=0;i<demo.length;i++){
        if(preCode==demo[i]){
            demo[i]=afterCode;
            return;
...
function showDemo(){
    if(demo.length<1){
        ...
        $("#tbody_y").html("");
        for(var i=0;i<demo.length;i++){
        $("#tbody_y").append("<tr><td>"+(i+1)+"</td><td id='t"+i+"'onclick='changeDemoCode(this)'>"+demo[i]+"</td><td><div class='btn-group'><input type='button' class='btn btn-info btn-xs' value='关联'/><input type='button' onclick='delFromDemo(this)' class='btn btn-danger btn-xs' value='删除'></div></td></tr>");
        }
        $(".demoCount").text(demo.length);
        $(".allCount").text(demo.length+preBreakNum);
    }
}
